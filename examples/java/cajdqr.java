...
public class cajdqr {
  ...
  public static void main(String argv[]) {
    ...
    if (argv.length != 3) {
	    System.out.println("Usage: java jdqr <trace-file-name> <ca-trace-file-name> <elf-file-name>");
	    System.exit(1);
    }

    System.out.printf("dqrdll version: %s\n",Trace.version());
 
    Trace t = new Trace(argv[0],true,argv[2],32,TraceDqr.AddrDisp.ADDRDISP_WIDTHAUTO.swigValue(),0);
    if (t == null) {
    ...
    rc = t.setCATraceFile(argv[1]);
    if (rc != TraceDqr.DQErr.DQERR_OK) {
      System.out.printf("setrCATraceFile(%s) is not OK\n",argv[1]);
      System.exit(1);
    }
...
//	  if (instInfo.getTimestamp() != 0) {
              System.out.printf("t:%d ",instInfo.getTimestamp());
	      System.out.printf("[%d] ",instInfo.getCycles());
//	  }
